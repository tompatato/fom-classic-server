"""Live memory access for the Face of Mankind (2006) client under Wine/Proton.

The Windows client runs as an ordinary Linux process under Wine, so its PE
modules show up as file-backed mappings in ``/proc/<pid>/maps``. We locate the
process by those mappings, recover each module's load base, and read/scan memory
through ``/proc/<pid>/mem``.

The 2006 client's PE module filenames are **not known until recon** (see
``docs/recon-plan.md``). Declare them once via the ``FOMC_CLIENT_MODULES`` env var
(comma-separated, e.g. ``FOMC_CLIENT_MODULES=client.exe,engine.dll``), or pass an
explicit ``modules=`` list — ``fomre`` supplies the modules already catalogued in
``disassembly/symbols/``. With none known, process detection reports a clear error
...
# The 2006 client's PE module filenames are unknown until recon catalogues them;
# there is no built-in default. See ``client_modules`` for how they are resolved.
_DEFAULT_CLIENT_MODULES: tuple[str, ...] = ()
...
def client_modules(modules: Sequence[str] | None = None) -> tuple[str, ...]:
    """Resolve the client's PE module filenames.

    Priority: an explicit ``modules`` argument (e.g. the programs catalogued in
    the symbol DB) > the ``FOMC_CLIENT_MODULES`` env var > the empty default.
    """
    if modules:
        return tuple(modules)
    env = os.environ.get("FOMC_CLIENT_MODULES")
    if env:
        return tuple(m.strip() for m in env.split(",") if m.strip())
    return _DEFAULT_CLIENT_MODULES


def _require_modules(modules: Sequence[str] | None) -> tuple[str, ...]:
    mods = client_modules(modules)
    if not mods:
        raise ProcessNotFoundError(
            "no client modules known — the 2006 client's PE module names haven't "
            "been catalogued yet. Set FOMC_CLIENT_MODULES (comma-separated) or "
            "populate disassembly/symbols/ (recon Step 0), then retry."
        )
    return mods
...
def find_client_pids(modules: Sequence[str] | None = None) -> list[int]:
    """Return PIDs whose mappings reference any known client module."""
    wanted = {n.lower() for n in _require_modules(modules)}
    hits = []
...
def find_client_pid(modules: Sequence[str] | None = None) -> int:
    mods = _require_modules(modules)
    pids = find_client_pids(mods)
    if not pids:
        raise ProcessNotFoundError(
            "no running FOM client found (looked for mappings of "
            f"{', '.join(mods)}). Launch the client first."
        )
    return pids[0]
...
def module_bases(pid: int, modules: Sequence[str] | None = None) -> dict[str, int]:
    out: dict[str, int] = {}
    mods = client_modules(modules)
...
def write_mem(pid: int, addr: int, data: bytes) -> int:
    """Write ``data`` at ``addr``. Guarded behind FOMC_RE_ALLOW_WRITE=1."""
    if os.environ.get("FOMC_RE_ALLOW_WRITE") != "1":
        raise MemoryAccessError(
            "memory writes are disabled; set FOMC_RE_ALLOW_WRITE=1 to enable"
        )
