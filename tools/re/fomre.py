#!/usr/bin/env python3
"""fomre — Face of Mankind (2006) reverse-engineering harness CLI.

Bridges the committed static symbol/type database (``disassembly/``) to the
live client process running under Wine/Proton. The examples below use
placeholder names — the real modules, symbols, and types are whatever recon
catalogues into ``disassembly/`` for the 2006 client.

Examples
--------
  ./fomre.py programs                        # modules + image bases
  ./fomre.py sym Player                      # search symbols
  ./fomre.py sym <SymbolName> --exact        # exact name -> address/RVA
  ./fomre.py type <TypeName>                 # struct layout (field offsets)
  ./fomre.py pid                             # find running client + bases
  ./fomre.py read <GlobalName> --type u32    # read a global as u32 (live)
  ./fomre.py read <module>:0xADDR --type /Type/Path
  ./fomre.py struct <module>:0xADDR /Type/Path
...
def cmd_pid(args):
    mods = list(_db().programs)
    try:
        pids = memory.find_client_pids(mods)
    except memory.ProcessNotFoundError as e:
        raise SystemExit(str(e))
    if not pids:
        raise SystemExit("no running FOM client found (launch it first)")
    for pid in pids:
        print(f"pid {pid}")
        for name, base in sorted(memory.module_bases(pid, mods).items()):
            print(f"  {name:<16} base={base:#010x}")


def cmd_maps(args):
    mods = memory.client_modules(list(_db().programs))
    pid = args.pid or memory.find_client_pid(mods)
    names = {n.lower() for n in mods}
    for r in memory.read_maps(pid):
        if os.path.basename(r.path).lower() in names:
            print(f"{r.start:#010x}-{r.end:#010x} {r.perms} {r.path}")


def _pid(args, modules=None) -> int:
    return args.pid or memory.find_client_pid(modules)


def cmd_read(args):
    db = _db()
    pid = _pid(args, list(db.programs))
