"""Static symbol/type database for the Face of Mankind (2006) client binaries.
...
The 2006 client's module set and image bases are established during recon (see
``docs/recon-plan.md``); until then ``disassembly/`` holds no data and the queries
return empty. Addresses in the symbol JSON are absolute *within the binary's image*
(Ghidra's view), so the file-relative offset is ``rva = addr - imageBase``. Each
module records its own image base in ``symbols/<program>/_meta.json``.
"""
...
DISASM_DIR = Path(os.environ.get("FOMC_DISASM_DIR", _REPO_ROOT / "disassembly"))
    ...
    def load(self) -> "SymbolDb":
        if not self.symbols_dir.is_dir():
            raise FileNotFoundError(
                ...
                f"(set FOMC_DISASM_DIR to override)"
            )
